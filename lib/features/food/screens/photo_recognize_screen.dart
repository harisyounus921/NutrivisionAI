...
class _PhotoRecognizeScreenState extends State<PhotoRecognizeScreen> {
  ...
  Future<void> _pickImage(ImageSource source) async {
    ...
    try {
      ...
      final candidates = await _foodService.recognizePhoto(
        bytes,
      ...
      if (candidates.isEmpty) {
        setState(() {
          _recognizing = false;
          _error =
              'No food recognized in this photo. Try a clearer image of a meal.';
        });
      } else {
        setState(() {
          _recognizing = false;
...
class _CandidatesList extends StatelessWidget {
  ...
  Color _confidenceColor(double confidence, Color fallback) {
