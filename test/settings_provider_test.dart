...
import 'package:ai_diet/core/services/notification_service.dart';
...
void main() {
  group('SettingsProvider.setMealRemindersEnabled', () {
    test('enabling when permission is granted turns on, schedules, and persists',
        () async {
    ...
    test('enabling when permission is denied stays off and does not schedule or persist',
        () async {
  ...
  group('SettingsProvider.loadSettings', () {
    test('reschedules reminders when stored on and OS still allows them',
        () async {
    ...
    test('clears the preference when stored on but OS permission was revoked',
        () async {
